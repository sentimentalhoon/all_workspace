...
const { fetchPostById, fetchComments, createComment, toggleLike, deletePost } =
  useBoard();
...
const canEdit = computed(() => post.value && (isAuthor.value || isAdmin.value));
...
const handleEdit = () => {
  if (!post.value) return;
  router.push(`/community/write?id=${post.value.id}`);
};
    ...
    <div v-else-if="post" class="content-wrapper glass-panel">
      ...
      <style scoped lang="scss">
        ...
        .post-header {
          ...
          /* .category-badge moved to header-top, remove margin-bottom from it if conflict */
          .category-badge {
            /* ... existing ... */
